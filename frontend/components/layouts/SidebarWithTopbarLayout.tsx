...
export function SidebarWithTopbarLayout({
  children,
  onLogout,
}: SidebarWithTopbarLayoutProps) {
  ...
  return (
    <div className="h-screen bg-muted/40 p-2 sm:p-3 md:p-4 flex gap-2 sm:gap-3 md:gap-4 overflow-hidden">
      <aside
        className={`hidden sm:flex bg-card rounded-2xl border shadow-sm flex-col flex-shrink-0 transition-all duration-300 ${sidebarCollapsed ? "w-16" : "w-56 md:w-60"}`}
      >
        <Sidebar
          onLogout={onLogout}
          variant="embedded"
